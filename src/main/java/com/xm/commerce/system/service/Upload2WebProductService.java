...
//    }
    ...
    public boolean upload2Shopify2(EcommerceProductStore productStore, EcommerceSite site, Integer uid, String singleKey) throws Exception {
        ...
        String querySKUUrl = site.getApi() + "?fields=variants";
        ResponseEntity<String> queryResponse = restTemplate.exchange(querySKUUrl, HttpMethod.GET, queryEntity, String.class);
        HttpHeaders headers = queryResponse.getHeaders();
        boolean skuExist = isSKUExist(queryResponse, productStore.getSku());
        if (skuExist){
        ...
        if (list != null && !list.isEmpty()){
            String link = switch2Url(list.get(0));
            boolean skuExistFurther = isSkuExistFurther(link, token, productStore.getSku());
            if (skuExistFurther) {
                throw new ProductSkuAlreadyExistException(ImmutableMap.of("sku", productStore.getSku()));
            }
        }
...
//        httpHeaders.set("Authorization", SHOPIFY_TOKEN);
    ...
    private boolean isSKUExist(ResponseEntity<String> queryResponse, String uploadSku){
        ...
        for (Object o : productArray) {
            JSONObject variantList = (JSONObject)o;
            JSONArray variantArray = variantList.getJSONArray("variants");
            JSONObject singleVariant = (JSONObject)variantArray.get(0);
            String sku = (String) singleVariant.get("sku");
            log.info("sku= " + sku);
            if (uploadSku.equals(sku)) {
    ...
    private String switch2Url(String link){
    ...
    private boolean isSkuExistFurther(String url, String token, String uploadSku){
        HttpHeaders queryHeaders = new HttpHeaders();
        queryHeaders.set("Authorization", token);
        HttpEntity<String> queryEntity = new HttpEntity<>(queryHeaders);
        ResponseEntity<String> queryResponse = restTemplate.exchange(url, HttpMethod.GET, queryEntity, String.class);
        List<String> links = queryResponse.getHeaders().get("Link");
        if (links == null || links.isEmpty()){
            return false;
        }
        String link = links.get(0);
        String nextUrl = switch2Url(link);
        if ("".equals(nextUrl)){
            return false;
        }
        boolean skuExist = isSKUExist(queryResponse, uploadSku);
        if (skuExist){
            return true;
        }
        return isSkuExistFurther(nextUrl,token,uploadSku);
    }
